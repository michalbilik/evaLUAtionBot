counter = 0
function LuckyKillerBobwhatTo( agent, actorKnowledge, time)
  --Zmienne
  ...
  changeWeapon = 0
	...
	-- no armor
	if (trig:getType() == Trigger.Armour and trig:isActive() and actorKnowledge:getArmour() == 0) then
		nav = actorKnowledge:getNavigation()
		for i = 0, nav:getNumberOfTriggers() - 1, 1 do
			trig = nav:getTrigger(i)
...
--Shotgun na start
function LuckyKillerBobonStart( agent, actorKnowledge, time)
